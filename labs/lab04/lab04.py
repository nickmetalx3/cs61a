...
def closer_city(lat, lon, city1, city2):
    ...
    cities = (city1, city2)
    c1 = (lat, lon)
    city_dic = {get_name(city): dist(c1, (get_lat(city), get_lon(city))) for city in cities}
    return min(city_dic, key = lambda t: city_dic[t])
...
def put_piece(board, max_rows, column, player):
    ...
    """
    column_index = column
    column_list = get_column_list(board)
    required_column = column_list[column_index] 
    for row in range(max_rows):
        if required_column[row] == '-':
            row += 1
        else:
            break
    row_index = row - 1
    if row_index != -1:
        new_row = replace_elem(board[row_index], column_index, player)
        board = replace_elem(board, row_index, new_row)
...
def make_move(board, max_rows, max_cols, col, player):
    ...
    -1
    """
    if col + 1 > max_cols or col + 1 < 0:
        return -1, board
    row_index, new_board = put_piece(board, max_rows, col, player)
...
>>>>>>> bfc3fd91812536f6f95293645f68ae9f53a4ce2e
